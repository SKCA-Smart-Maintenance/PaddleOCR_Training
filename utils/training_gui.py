...
MAX_LOG_LINES      = 1000
DEFAULT_SETUP_URL  = "https://raw.githubusercontent.com/SKCA-Smart-Maintenance/PaddleOCR_Training/main/utils/setup_notebook.py"
_UNBUFFERED_ENV    = {**os.environ, "PYTHONUNBUFFERED": "1"}
_processes         = {"setup": None, "train": None, "eval": None, "export": None, "convert": None}
...
def section_header(text):
    return widgets.HTML(f'<div class="section-header">{text}</div>')
...
def download_link_html(zip_path: str) -> str:
    ...
    return (
        f'<a href="/kaggle/working/{zip_path}" download="{filename}" target="_blank" '
        f'style="display:inline-block;margin-top:8px;padding:6px 14px;background:#1565c0;'
        f'color:#fff;border-radius:5px;font-weight:600;text-decoration:none;font-size:12px">'
        f'⬇  Download {filename}</a>'
    )

def make_log(height="360px") -> widgets.Textarea:
    ta = widgets.Textarea(value="", layout=Layout(width="100%", height=height))
...
# ═══════════════════════════════════════════════════════════════════════════════
def run_command(cmd, log, proc_key, on_done_msg="✅ Done", on_finish_cb=None):
    def _stream():
        try:
            proc = subprocess.Popen(
                cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                text=True, bufsize=1, env=_UNBUFFERED_ENV,
            )
...
# ── Widgets ───────────────────────────────────────────────────────────────────
setup_url_input = widgets.Text(
    value=DEFAULT_SETUP_URL,
    description="Script URL:",
    style=LABEL_STYLE,
    layout=Layout(flex="1 1 auto", height="32px"),
)
setup_save_dir_input = widgets.Text(
    value=".",
    description="Save Dir:",
    style={"description_width": "80px"},
    layout=Layout(width="180px", height="32px"),
)

wget_btn         = make_btn("⬇  Download Script", "download",    "#E3F2FD", "200px")
setup_btn        = make_btn("⚙️  Setup PaddleOCR", "cogs",        "#E8F5E9", "200px")
check_btn        = make_btn("🔍  Check System",    "check-circle","#FFF8E1", "200px")
setup_stop_btn   = make_btn("■  Stop",             "stop",        "#FFEBEE", "120px")
setup_stop_btn.disabled = True
setup_status_label = widgets.HTML(value="")
setup_log          = make_log("420px")

# Track whether the script has been imported
_setup_module = None

# ── Layout ────────────────────────────────────────────────────────────────────
setup_tab = VBox([
    section_header("📥  Setup Script"),
    HBox(
        [setup_url_input, setup_save_dir_input, wget_btn],
        layout=Layout(width="100%", align_items="center", gap="8px"),
    ),
    section_header("🚀  Run Setup"),
    HBox(
        [setup_btn, check_btn,
         widgets.HTML("<div style='flex:1'></div>"),
         setup_status_label, setup_stop_btn],
        layout=Layout(width="100%", align_items="center", gap="8px"),
    ),
    setup_log,
...
def _get_script_path() -> str:
    """Resolve the local path of the downloaded setup script."""
    save_dir  = setup_save_dir_input.value.strip() or "."
    filename  = os.path.basename(setup_url_input.value.strip())
    return os.path.join(save_dir, filename)


def _import_setup_module(script_path: str):
    """
    Dynamically import (or re-import) the setup script from its file path.
    Using importlib.util lets us load it from any directory without
    modifying sys.path permanently.
    """
    global _setup_module
    spec          = importlib.util.spec_from_file_location("setup_notebook", script_path)
...
def _run_in_thread(fn, log, status_label, done_msg="✅ Done"):
    """
    Run a plain Python callable in a daemon thread, capture stdout/stderr
    by redirecting them to a StringIO-like forwarder, then restore.
    Also handles the start/stop button states.
    """
    import io

    class _ForwardingWriter(io.IOBase):
        """Forwards write() calls to log_append so print() output appears live."""
        def write(self, s):
            if s:
                log_append(log, s)
    ...
    def _thread():
        ...
        setup_stop_btn.disabled   = False
        status_label.value        = status_html("⏳ Running…", "#e65100")

        old_stdout, old_stderr = sys.stdout, sys.stderr
        writer = _ForwardingWriter()
        sys.stdout = writer
        sys.stderr = writer
        try:
            fn()
            log_append(log, f"\n{'─'*60}\n{done_msg}\n")
            status_label.value = status_html("✅ Done", "#2e7d32")
        except Exception as exc:
            log_append(log, f"\n❌  Error: {exc}\n")
            status_label.value = status_html("❌ Error", "#c62828")
        finally:
...
# ── Handlers ─────────────────────────────────────────────────────────────────
def on_wget(_):
    """Download the setup script with wget -nc (skip if already exists)."""
    url      = setup_url_input.value.strip()
    save_dir = setup_save_dir_input.value.strip() or "."
    if not url:
        log_clear(setup_log)
        log_append(setup_log, "⚠️  Please enter a script URL.\n"); return
    ...
    def _on_done():
        ...
        if os.path.isfile(script_path):
            log_append(setup_log, f"📄  Script ready at: {script_path}\n")
            setup_status_label.value = status_html("✅ Downloaded", "#2e7d32")
        setup_btn.disabled      = False
        check_btn.disabled      = False
        setup_stop_btn.disabled = True
    ...
    check_btn.disabled        = True
    setup_stop_btn.disabled   = False
    setup_status_label.value  = status_html("⏳ Downloading…", "#e65100")

    run_command(
        ["wget", "-nc", "-P", save_dir, url],
        setup_log, "setup",
        on_done_msg="✅ Download finished",
        on_finish_cb=_on_done,
    )


def on_setup_paddleocr(_):
    """Import the script then call setup_notebook.setup_paddleocr()."""
    script_path = _get_script_path()
    if not os.path.isfile(script_path):
        log_clear(setup_log)
        log_append(setup_log, f"⚠️  Script not found at '{script_path}'. Download it first.\n"); return

    log_clear(setup_log)
    log_append(setup_log, f"📦  Importing {script_path} …\n" + "─" * 60 + "\n")

    try:
        mod = _import_setup_module(script_path)
    except Exception as exc:
        log_append(setup_log, f"❌  Import failed: {exc}\n"); return

    if not hasattr(mod, "setup_paddleocr"):
        log_append(setup_log, "❌  setup_notebook.setup_paddleocr() not found in script.\n"); return

    _run_in_thread(mod.setup_paddleocr, setup_log, setup_status_label,
                   done_msg="✅ setup_paddleocr() finished")


def on_check_system(_):
    """Import the script then call setup_notebook.check_system()."""
    script_path = _get_script_path()
    if not os.path.isfile(script_path):
        log_clear(setup_log)
        log_append(setup_log, f"⚠️  Script not found at '{script_path}'. Download it first.\n"); return
    ...
    if not hasattr(mod, "check_system"):
        log_append(setup_log, "❌  setup_notebook.check_system() not found in script.\n"); return

    _run_in_thread(mod.check_system, setup_log, setup_status_label,
                   done_msg="✅ check_system() finished")


def on_stop_setup(_):
    """Stop wget/subprocess if running; Python fn threads can't be force-killed."""
    proc = _processes.get("setup")
    if proc and proc.poll() is None:
        proc.terminate()
        log_append(setup_log, "\n🛑  Stopped by user.\n")
    setup_status_label.value = status_html("🛑 Stopped", "#c62828")
    setup_btn.disabled       = False
    check_btn.disabled       = False
    setup_stop_btn.disabled  = True


wget_btn.on_click(on_wget)
setup_btn.on_click(on_setup_paddleocr)
check_btn.on_click(on_check_system)
setup_stop_btn.on_click(on_stop_setup)
...
export_link              = widgets.HTML(value="")
...
def on_export(_):
    ...
    cmd = [
        "python3", "-u", EXPORT_SCRIPT, "-c", CONFIG_SAVE_PATH,
        "-o",
...
def on_download_export(_):
    ...
    run_command(["zip", "-rq", zip_path, save_dir], export_log, "export",
                "✅ Zip finished",
                on_finish_cb=lambda: setattr(export_link, "value", download_link_html(zip_path)))
...
convert_link              = widgets.HTML(value="")
...
def on_download_onnx(_):
    ...
    run_command(["zip", "-rq", zip_path, onnx_dir], convert_log, "convert",
                "✅ Zip finished",
                on_finish_cb=lambda: setattr(convert_link, "value", download_link_html(zip_path)))
...
task_tab = widgets.Tab(
    children=[setup_tab, training_tab, evaluation_tab, export_model_tab, convert_model_tab],
    layout=Layout(width="100%"),
)
for i, title in enumerate([
    "🔧  Setup", "🚂  Training", "🧪  Evaluation", "📦  Export Model", "🔄  Convert Model",
]):
